...
export default function Configurator() {
  ...
  return (
    <div className={`w-screen bg-[#F0EFEd] relative flex flex-col ${viewMode === "building" ? "min-h-screen overflow-y-auto" : "h-screen overflow-hidden"}`}>
      ...
      {/* ── DESKTOP TOP BAR ── */}
      {!isMobile && (
        <div className={`${viewMode === "building" ? "sticky" : "absolute"} top-0 left-0 right-0 z-30 flex items-center px-4 py-4 bg-white/80 backdrop-blur border-b border-gray-200 overflow-x-auto gap-4 min-w-0`}>
      ...
      {/* ── WORKSPACE ── */}
      <div className={`${viewMode === "building" ? "relative flex" : "flex-1 relative flex min-h-0"} ${isMobile ? "pt-12" : "pt-16"}`}>
        {viewMode === "elevations" ? (
