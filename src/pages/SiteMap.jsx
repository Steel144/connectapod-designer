...
export default function SiteMap() {
  ...
  const [saveDetails, setSaveDetails] = useState(() => {
    try { 
      ...
      return saved || { projectName: '', clientName: '', address: '' }; 
    } catch { 
  ...
  useEffect(() => {
    ...
    const loadImageForFurniture = (furn) => new Promise((resolve) => {
      if (!furn.image) { resolve({ furn, img: null }); return; }
      const img = new window.Image();
      img.crossOrigin = 'anonymous';
      img.onload = () => resolve({ furn, img });
      img.onerror = () => resolve({ furn, img: null });
      img.src = furn.image;
    });
      ...
      <div className="flex items-center px-4 py-4 bg-white border-b border-gray-200 overflow-x-auto gap-4 min-w-0 z-30">
        ...
        <div className="flex items-center gap-2 ml-auto shrink-0">
          ...
          <div className="flex border border-gray-200 overflow-hidden shrink-0">
            ...
            <button onClick={() => setShowSaveModal(true)} disabled={!address} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs border transition-all ${address ? 'bg-white text-gray-600 border-gray-200 hover:border-[#F15A22] hover:text-[#F15A22]' : 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed'}`} style={{ clipPath: "polygon(0 0, calc(100% - 6px) 0, 100% 50%, calc(100% - 6px) 100%, 0 100%)" }} title={address ? '' : 'Enter site address to save'}>
              <Save size={13} /> Save
            </button>
