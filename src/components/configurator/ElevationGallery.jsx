    ...
    <div className="w-full h-full bg-white overflow-auto p-3">
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {wallsWithElevations.map((wall) => (
          <div key={wall.id} className="bg-black rounded overflow-hidden shadow hover:shadow-lg transition-shadow">
            ...
            <div className="p-2 bg-gray-800">
              <p className="text-white font-semibold text-xs truncate">{wall.label || wall.type}</p>
              <p className="text-gray-400 text-[10px] mt-0.5">Face: {wall.face || 'N/A'}</p>
            </div>
          </div>
        ))}
      </div>
