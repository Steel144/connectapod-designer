  ...
  const VertElevation = ({ layers, label, color }) => {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ position: "relative", width: Math.max(totalDepthPx, Math.round(scale * PAV_END_WIDTH_M * PX_PER_M)), height: wallHPx, border: "1px solid #e5e7eb", backgroundColor: "#f9fafb", overflow: "hidden" }}>
          {layers.map((layer) =>
            layer.slots.map((slot, si) => {
